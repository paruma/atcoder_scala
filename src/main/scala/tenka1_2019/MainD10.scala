  ...
  */
  ...
  def solve(n: Int, a: IndexedSeq[Int]): Long = {
    ...
    val numNotTriangle = {
      val s = a.sum
      val phiInvT1 = {
        val dp1Head = IndexedSeq.tabulate(s + 1)(x => if (x == 0) ResidueRing(1) else ResidueRing(0))
        val dp1Tail = a.foldLeft(dp1Head)((dp1, ae) =>
          IndexedSeq.tabulate(s + 1)(x =>
            ResidueRing(2) * dp1(x) + (if (x - ae >= 0) dp1(x - ae) else ResidueRing(0))
          ))
        val range = if (s % 2 == 0) s / 2 to s else s / 2 + 1 to s
        range.map(x => dp1Tail(x)).foldLeft(ResidueRing(0))(_ + _)
      }
      val phiInvT23 = {
        if (s % 2 != 0) ResidueRing(0)
        else {
          val dp23Head = IndexedSeq.tabulate(s / 2 + 1)(x => if (x == 0) ResidueRing(1) else ResidueRing(0))
          val dp23Tail = a.foldLeft(dp23Head)((dp23, ae) =>
            IndexedSeq.tabulate(s / 2 + 1)(y =>
              (if (y - ae >= 0) dp23(y - ae) else ResidueRing(0)) + dp23(y)
            ))
          dp23Tail(s / 2)
        }
      }
