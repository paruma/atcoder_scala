...
object MainD5 {
  ...
  */
  ...
  // ナイーブ実装
  def pow3(n: Int): ResidueRing = (0 until n).foldLeft(1)((acc, _) => rrtimes3(acc))
  ...
  def solve(n: Int, a: IndexedSeq[Int]): ResidueRing = {
    ...
    val numNotTriangle = {
      val s = a.sum
      val phiInvT1 = {
        val dp1: Array[Array[ResidueRing]] = Array.fill(n + 1)(Array.fill(s + 1)(0))
        dp1(0)(0) = 1
        for (k <- 1 to n; x <- 0 to s) {
          dp1(k)(x) = rrplus(rrtimes2(dp1(k - 1)(x)),
            if (x - a(k - 1) >= 0) dp1(k - 1)(x - a(k - 1)) else 0)
        }
        val range = if (s % 2 == 0) s / 2 to s else s / 2 + 1 to s
        range.map(x => dp1(n)(x)).foldLeft(0)((acc, x) => rrplus(acc, x))
      }
      val phiInvT23 = {
        if (s % 2 != 0) 0
        else {
          val dp23: Array[Array[ResidueRing]] = Array.fill(n + 1)(Array.fill(s / 2 + 1)(0))
          dp23(0)(0) = 1
          for (k <- 1 to n; y <- 0 to s / 2) {
            dp23(k)(y) = rrplus(if (y - a(k - 1) >= 0) dp23(k - 1)(y - a(k - 1)) else 0, dp23(k - 1)(y))
          }
          dp23(n)(s / 2)
        }
      }
