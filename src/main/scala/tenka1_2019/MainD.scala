...
object MainD {
  ...
  */
  ...
  // ナイーブ実装
  def pow(base: Int, n: Int): Int = List.fill(n)(base).product

  def solve(n: Int, a: IndexedSeq[Int]): Int = {
    val numAll = pow(3, n)
    val numNotTriangle = {
      val s = a.sum
      val phiInvT1 = {
        val dp1: Array[Array[Int]] = Array.fill(n + 1)(Array.fill(s + 1)(0))
        dp1(0)(0) = 1
        for (k <- 1 to n; x <- 0 to s) {
          dp1(k)(x) = 2 * dp1(k - 1)(x)  + (if (x - a(k - 1) >= 0) dp1(k - 1)(x - a(k - 1)) else 0)
        }
      ...
      val phiInvT23 = {
        if (s % 2 != 0) 0
          ...
          val cums = a.scanLeft(0)(_ + _)
          val dp23: Array[Array[Int]] = Array.fill(n + 1)(Array.fill(s / 2 + 1)(0))
          dp23(0)(0) = 1
